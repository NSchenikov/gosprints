...
// Конкурентность на уровне процессора и операционной системы реализовывается с помощью потоков. Существует возможность создавать потоки и выполнять в них всевозможные операции. Golang не дает возможность напрямую создавать потоки, но в место этого предоставляет конструкцию, которая называется горутиной. Потоки имеют фиксированный размер стека, который определяется при компиляции программы. В golang он определяется в рантайме. Горутины - это легковесные потоки. Горутины - это абстракции и "под капотом" они все равно распределяются по потокам. За этот процесс отвечает планировщик. При создании горутины планировщик ставит ее в очередь на выполнение. Main - главная горутина. Если мы ставим time.Sleep, то она блокирует выполнение главной горутины (функции Main) и планировщик переключается на другую ранее запланированную горутину. Каналы нужны для синхронизации и обмена данными между горутинами. Горутины могут писать данные в канал и читать с него. Чтение и запись с канала блокируют выполнение горутин. Буферизированный канал не блокирует выполнение при записи. 